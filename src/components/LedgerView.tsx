...
export default function LedgerView({
  ...
  const activeAccount = useMemo(() => {
    return accounts.find(a => a.id === selectedLedgerId) || accounts[0];
  ...
  const openingBalance = useMemo(() => {
    if (activeAccount.id === 'hdfc-bank') return 840250.00;
    if (activeAccount.id === 'cash-in-hand') return 187200.00;
    return activeAccount.balance * 0.9;
  }, [activeAccount]);
  ...
  const ledgerRows = useMemo(() => {
    ...
    const relevantVouchers = vouchers.filter(v => {
      ...
      return v.items.some(item => item.accountId === activeAccount.id);
    });
    ...
    const sortedVouchers = [...relevantVouchers].sort((a,b) => new Date(a.date).getTime() - new Date(b.date).getTime());

    sortedVouchers.forEach(vch => {
      // Find matching item index
      const item = vch.items.find(i => i.accountId === activeAccount.id);
      if (!item) return;
      ...
      // AWS (Payment) shows 45000 Debit. Decreased balance from 840250 to 795250.
      // TechNova (Receipt) shows 520000 Credit. Increased balance from 795250 to 1315250.
      if (dr !== null) {
      ...
      rows.push({
        date: vch.date,
        particulars: vch.particulars ?? vch.narration,
        vchType: vch.vchType,
        vchNo: vch.vchNo,
        debit: dr,
        credit: cr,
