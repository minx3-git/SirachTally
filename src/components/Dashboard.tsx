...
export default function Dashboard({
  ...
  const accReceivable = useMemo(() => accounts.find(a => a.id === 'acc-receivable')?.balance ?? 3771940.00, [accounts]);
  const accPayable = useMemo(() => accounts.find(a => a.id === 'acc-payable')?.balance ?? 1071506.35, [accounts]);
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        >
          ...
          <div className="mt-4">
            ...
            <h2 className="text-2xl font-bold tracking-tight text-slate-900 mt-1 tabular-numbers">₹{cashInHand.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</h2>
          </div>
        </div>
        ...
        >
          ...
          <div className="mt-4">
            ...
            <h2 className="text-2xl font-bold tracking-tight text-slate-900 mt-1 tabular-numbers">₹{bankBalance.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</h2>
          </div>
        </div>
        ...
        >
          ...
          <div className="mt-4">
            ...
            <h2 className="text-2xl font-bold tracking-tight text-slate-900 mt-1 tabular-numbers">₹{accReceivable.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</h2>
          </div>
        </div>
        ...
        >
          ...
          <div className="mt-4">
            ...
            <h2 className="text-2xl font-bold tracking-tight text-slate-900 mt-1 tabular-numbers">₹{accPayable.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</h2>
          </div>
        </div>
          ...
          <div className="h-68 w-full relative">
            ...
            {hoveredPoint && (
              ...
              >
                ...
                <p className="mt-1 text-sky-400">Income: <span className="font-bold text-white">₹{hoveredPoint.income.toLocaleString('en-IN')}</span></p>
                <p className="text-emerald-400">Expense: <span className="font-bold text-white">₹{hoveredPoint.expense.toLocaleString('en-IN')}</span></p>
              </div>
            )}
          </div>
        </div>
                ...
                filteredTransactions.map((tx, idx) => (
                  ...
                  >
                    ...
                    <td className={`px-5 py-3.5 text-right font-mono font-semibold text-sm tracking-tight border-none tabular-numbers ${
                      ...
                      {tx.vchType === 'Payment' || tx.narration.toLowerCase().includes('withdraw') || tx.items[0].credit !== null ? '-' : '+'}
                      ₹{(tx.amountValue ?? 0).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                    </td>
