...
export default function MasterLedgerCosts({
  ...
  const handleAddAccountSubmit = (e: React.FormEvent) => {
    ...
    onAddAccount({
      id: editingAccountId || newAccName.toLowerCase().replace(/\s+/g, '-'),
      name: newAccName,
      code: newAccCode,
      type: newAccType,
      balance: bal,
    ...
    alert(editingAccountId ? 'Ledger account details and balances updated successfully!' : 'Custom ledger added successfully to account catalogs!');
  };
            ...
            <div>
              <div className="flex justify-between items-center border-b border-slate-100 pb-2 mb-4">
                <h3 className="text-xs font-extrabold text-slate-800 uppercase tracking-wider flex items-center gap-1.5">
                  <FolderPlus className={`h-4 w-4 ${editingAccountId ? 'text-amber-500' : 'text-emerald-600'}`} />
                  {editingAccountId ? 'Modify Ledger Account' : 'Add Ledger Account'}
                </h3>
                {editingAccountId && (
                  <button 
                    type="button" 
                    ...
                    className="flex items-center gap-1 text-[10px] bg-slate-100 hover:bg-rose-50 hover:text-rose-600 text-slate-500 font-extrabold px-2 py-0.5 rounded transition-all cursor-pointer"
                  >
                    <X className="h-3 w-3" />
                    <span>Cancel</span>
                  </button>
              ...
              <form onSubmit={handleAddAccountSubmit} className="space-y-3.5">
                <div>
                  ...
                  <input 
                    ...
                    disabled={!!editingAccountId && (editingAccountId === 'hdfc-bank' || editingAccountId === 'cash-in-hand')}
                    className="w-full text-xs px-3 py-2 border border-slate-200 rounded-lg focus:outline-none focus:border-[#00236f] disabled:bg-slate-50 disabled:text-slate-500 font-semibold"
                  />
                  {editingAccountId && (editingAccountId === 'hdfc-bank' || editingAccountId === 'cash-in-hand') && (
                    <span className="text-[9px] text-amber-600 font-bold mt-1 block">Primary system core keys cannot be renamed.</span>
                  )}
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input 
                      ...
                      className="w-full text-xs px-3 py-2 border border-slate-200 rounded-lg focus:outline-none focus:border-[#00236f] font-bold"
                    />
                  </div>
                  <div>
                ...
                <button 
                  type="submit"
                  className={`w-full py-2.5 text-white font-extrabold text-[11px] uppercase tracking-widest rounded-lg transition-all cursor-pointer shadow-3xs text-center ${
                    editingAccountId ? 'bg-amber-600 hover:bg-amber-700' : 'bg-[#00236f] hover:bg-brand-primary'
                  }`}
                >
                  {editingAccountId ? 'Save Changes & Balance' : 'Create Master Ledger'}
                </button>
              </form>
            </div>
                      ...
                      <td className="py-3.5 px-4 text-center">
                        <button
                          ...
                          onClick={() => {
                            ...
                            setNewAccBalanceType(acc.balanceType);
                            setNewAccDetails(acc.details || '');
                            // Scroll side panel into view if on narrow screens
                            document.querySelector('.h-full')?.scrollTo({ top: 0, behavior: 'smooth' });
                          }}
                          className="px-2 py-1 bg-slate-50 border border-slate-200 text-[#00236f] hover:bg-[#00236f] hover:text-white rounded-lg text-[10px] font-bold inline-flex items-center gap-1 transition-all cursor-pointer"
                        >
