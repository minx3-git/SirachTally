...
export default function Login({ onLoginSuccess, onBypassLogin }: LoginProps) {
  ...
  const handleEmailSignIn = async (e: React.FormEvent) => {
    ...
    // 1. First search local sandbox repository
    ...
    // 2. Fall back to remote Firebase Auth
    try {
      ...
      if (err.code === 'auth/operation-not-allowed') {
        // Since Email/Password auth is disabled, check if this email exists under sandbox with an incorrect password
        const emailExistsInSandbox = localSandboxUsers.some(u => u.email.toLowerCase() === email.toLowerCase());
        if (emailExistsInSandbox) {
          friendlyError = 'Incorrect password specified for this sandbox presentation account. Please check details and retry.';
        } else {
          friendlyError = 'Security Notice: Email/Password authentication is disabled in Firebase. However, you can register new accounts instantly under the "SIGN UP" tab above without any configuration!';
        }
      }
  ...
  const handleEmailSignUp = async (e: React.FormEvent) => {
    ...
    // Helper to generate local sandbox profile
    const performSandboxSignup = () => {
      console.log("Activating auto-sandbox signup for presentation ease.");
      const sandboxUid = 'bypass-' + Math.random().toString(36).substring(2, 9);
      
      const isSuperAdminEmail = 
        email === 'minakshivinod333@gmail.com' || 
        email.toLowerCase().includes('admin') ||
        email.toLowerCase().includes('sirach');

      const assignedRole = isSuperAdminEmail ? 'Head Accountant' : requestedRole;
      ...
      const sandboxProfile = {
        ...
        name: fullName,
        role: assignedRole,
        password: password, // Store password to support simulated logins
        isBypass: true,
        updatedAt: new Date().toISOString()
      };

      // Read, append and write back
      const localList: any[] = JSON.parse(localStorage.getItem('sirach_sandbox_users') || '[]');
      // Filter out pre-existing matching emails in sandbox
      const filteredList = localList.filter(u => u.email.toLowerCase() !== email.toLowerCase());
      filteredList.push(sandboxProfile);
      localStorage.setItem('sirach_sandbox_users', JSON.stringify(filteredList));

      // Save as active presentation session
      localStorage.setItem('sirach_active_bypass_user', JSON.stringify({ user: sandboxUser, profile: sandboxProfile }));

      onLoginSuccess(sandboxUser, assignedRole);
    };
    ...
    try {
      ...
      if (err.code === 'auth/operation-not-allowed') {
        // Firebase Email/password is disabled. Transparently create sandbox account so the user never fails!
        performSandboxSignup();
        return;
      }
