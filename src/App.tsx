...
export default function App() {
  ...
  // Callback to append or update standard ledger account
  const handleAddLedgerAccount = async (newAccount: LedgerAccount) => {
    if (profile.role === 'Auditor' || profile.role === 'Client') {
      alert(`Permission Denied: ${profile.role} accounts are restricted to read-only access.`);
      return;
    }
    const isEditing = accounts.some(a => a.id === newAccount.id);
    setIsSyncing(true);
    try {
      ...
      await logAuditPayload(
        ...
        'LedgerMaster', 
        isEditing 
          ? `Updated ledger account details for ${newAccount.name}` 
          : `Onboarded new ledger account of segment ${newAccount.type}: ${newAccount.name}`
      );
    } catch (error) {
    ...
    try {
      ...
      if (err.code === 'auth/operation-not-allowed') {
        alert(
          ...
          "To enable real Firebase syncing for Quick-Pass profiles:\n" +
          "1. Go to Firebase Console -> Authentication -> Sign-in method\n" +
          "2. Enable 'Email/Password' provider.\n\n" +
          "Falling back to Local Sandbox Mode for presentation."
        );
        // Fallback to local sandbox mock user only if they haven't enabled Email/Password yet
        const bypassId = 'bypass-' + Math.random().toString(36).substring(2, 9);
        const mockUser = { uid: bypassId, displayName: name, email, isBypass: true };
        ...
        localStorage.setItem('sirach_active_bypass_user', JSON.stringify({ user: mockUser, profile: mockProfile }));
        setCurrentUser(mockUser);
        setUserProfile(mockProfile);
      } else {
        alert(`Bypass Authentication Error: ${err.message || err}`);
      }
    } finally {
      setIsLoading(false);
    }
        ...
        <div>
          ...
          <nav className="space-y-0.5 px-2">
            {[
              ...
              { id: 'payroll', label: 'Enterprise Payroll', icon: Users },
            ...
            <>
              ...
              {currentTab === 'banking' && (
                <BankingHub 
                  ...
                  onUpdateAccount={handleAddLedgerAccount}
